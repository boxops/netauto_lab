...
async def run_async_simple_client(comm, host, port, framer=FramerType.SOCKET):
    ...
    try:
        rr = await client.read_holding_registers(100, count=2, slave=1)
    ...
    try:
        ...
        rr = await client.read_holding_registers(110, count=2, slave=1)
    except ModbusException as exc:
        print(f"Received ModbusException({exc}) from library")
        client.close()
        return
...
if __name__ == "__main__":
    asyncio.run(run_async_simple_client("tcp", "10.17.1.99", 502), debug=True)
