...
from custom_jobs.custom_monitoring.swisclient import SwisClient
...
class SolarWinds:
    ...
    def get_node_id(self, node_name):
        ...
        self.logger.info(f"get_node_id - node_id query results: {node_id}.")
    ...
    def get_node_uri(self, node_name):
        ...
        self.logger.info(f"get_node_uri - node uri query results: {node_uri}.")
        if node_uri["results"]:
            return node_uri["results"][0]["Uri"]
        else:
            return ""

    def add_node_using_snmp_v2(self, node_name, ip_address, snmp_community):
        """Creates a new node using the supplied name an IP address.  Configure with our standard SNMPv2 credentials.
        ...
        Args:
            ...
            snmp_community(string): The SNMP community string that will be associated with the node object.
        ...
        if not self.does_node_exist(node_name):
            ...
            self.logger.info(f"add_node - add node invoke results: {results}.")
    ...
    def is_poller_attached_to_node(self, node_name, poller_name):
        ...
        self.logger.info(
            f"is_poller_attached_to_node - check for poller query results: {results}."
        )
    ...
    def attach_poller_to_node(self, node_name, poller_name, enabled=True):
        ...
        if not self.is_poller_attached_to_node(node_name, poller_name):
            ...
            self.logger.info(
                f"attach_poller_to_node - poller create results: {results}."
            )
    ...
    def enable_hardware_health_on_node(self, node_name):
        ...
        results = self.swis.invoke(
            "Orion.HardwareHealth.HardwareInfo", "EnableHardwareHealth", net_object, 9
        )
        self.logger.info(
            f"enable_hardware_health - enable hardware health invoke results: {results}."
        )
    ...
    def add_node_to_ncm(self, node_name):
        ...
        results = self.swis.invoke(
            "Cirrus.Nodes", "AddNodeToNCM", self.get_node_id(node_name)
        )
        self.logger.info(
            f"add_node_to_ncm - add node to ncm invoke results: {results}."
        )
    ...
    def add_node_to_udt(self, node_name):
        ...
        self.logger.info(
            f"add_node_to_udt - add node at l2 to udt create results: {results}."
        )
        ...
        results = self.swis.create("Orion.UDT.NodeCapability", **udt_properties)
        self.logger.info(
            f"add_node_to_udt - add node at l3 to udt create results: {results}."
        )
    ...
    def add_node_to_ip_vnqm(self, node_name):
        ...
        self.logger.info(
            f"add_node_to_vnqm - add node to vnqm create results: {results}."
        )

    def add_icmp_echo_ip_sla_operation_to_node(
        self, node_name, ip_sla_operation_number, ip_sla_name
    ):
        ip_sla_properties = {
            "NodeID": self.get_node_id(node_name),
            "OperationTypeID": 5,
        ...
        self.logger.info(
            f"add_icmp_echo_ip_sla_operation_to_node - add IP SLA operation to node create results: {results}."
        )
    ...
    def add_group_custom_property(self, property_name, description, value_type, size):
        ...
        self.logger.info(
            f"add_group_custom_property - add group custom property results: {results}."
        )

    def set_group_custom_property(
        self, group_name, custom_property_name, custom_property_value
    ):
    ...
    def set_node_custom_property(
        self, node_name, custom_property_name, custom_property_value
    ):
    ...
    def get_node_custom_properties(self, node_name):
        ...
        self.logger.info(
            f"set_custom_properties - custom_properties read results: {custom_properties}."
        )
    ...
    def get_list_of_custom_pollers_for_node(self, node_name):
        ...
        self.logger.info(
            f"get_list_of_custom_pollers_by_name - custom_pollers query results: {custom_pollers}."
        )
    ...
    def remove_custom_poller_by_name(self, node_name, poller_name):
        ...
        self.logger.info(
            f"remove_custom_poller_by_name - custom_poller_id query results: {custom_poller_id}."
        )
        ...
        self.logger.info(
            f"remove_custom_poller_by_name - custom_poller_uri query results: {custom_poller_uri}."
        )
    ...
    def add_custom_poller_by_name(self, node_name, poller_name):
        ...
        self.logger.info(
            f"add_custom_poller_by_name - custom_poller_id query results: {custom_poller_id}."
        )
    ...
    def poll_now(self, node_name):
        """For a given node, polls the node immediately.
        ...
        results = self.swis.invoke("Orion.Nodes", "PollNow", node_id)
        self.logger.info(f"poll_now - poll now invoke results: {results}.")
    ...
    def get_group_id(self, group_name):
        ...
        self.logger.info(f"get_group_id - group_id query results: {group_id}.")
    ...
    def get_group_uri(self, group_name):
        ...
        self.logger.info(f"get_group_uri - group_uri query results: {group_uri}.")
    ...
    def add_group(
        ...
        if not self.does_group_exist(group_name):
            ...
            self.logger.info(f"add_group - add group invoke results: {results}.")
    ...
    def is_node_in_group(self, node_name, group_name):
        ...
        self.logger.info(
            f"is_node_in_group - is_node_in_group query results: {results}."
        )
    ...
    def get_dependency_id(self, dependency_name):
        ...
        self.logger.info(
            f"get_dependency_id - dependency_id query results: {dependency_id}."
        )
    ...
    def get_list_of_interfaces(self, node_name):
        ...
        list_interfaces_names = self.swis.query(
            "SELECT Name FROM Orion.NPM.Interfaces WHERE NodeID " "= @node_id",
            node_id=node_id,
        )

        self.logger.info(
            f"get_list_of_interfaces_by_name - list_of_interface_names query results: {list_interfaces_names}."
        )
    ...
    def remove_interface(self, node_name, interface_name):
        ...
        if self.does_interface_exist(node_name, interface_name):
            self.logger.info(
                f"remove_interface_by_name - interface_uri query results: {interface_uri}."
            )
    ...
    def get_discovered_interfaces(self, node_name):
        ...
        discovered_interfaces = self.swis.invoke(
    ...
    def add_interface(self, node_name, interface_name):
        ...
        discovered_interface = [
            x for x in discovered_interfaces if x["Caption"].startswith(interface_name)
        ]
    ...
    def ncm_download_nodes_running_config(self, node_name):
        ...
        self.swis.invoke(
            "Cirrus.ConfigArchive", "DownloadConfig", [cirrus_node_id], "Running"
        )
    ...
    def delete_node(self, node_name):
        """Delete a node from SolarWinds.

        Args:
            node_name(string): The name of the node to be deleted.
        ...
        self.logger.info(f"delete_node - node {node_name} deleted.")

    def unmanage_node(self, node_name):
        """Unmanage a node in SolarWinds.

        Args:
            node_name(string): The name of the node to unmanage.
        ...
        node_id = self.get_node_id(node_name)
        netObjectId = "N:{}".format(node_id)
        now = datetime.utcnow()
        tomorrow = now + timedelta(days=1)
        self.swis.invoke("Orion.Nodes", "Unmanage", netObjectId, now, tomorrow, False)
        # self.swis.invoke("Orion.Nodes", "Unmanage", netObjectId)
        self.logger.info(f"unmanage_node - node {node_name} unmanaged.")

    def remanage_node(self, node_name):
        """Remanage a node in SolarWinds.

        Args:
            node_name(string): The name of the node to remanage.
        ...
        node_id = self.get_node_id(node_name)
        netObjectId = "N:{}".format(node_id)
        self.swis.invoke("Orion.Nodes", "Remanage", netObjectId)
        self.logger.info(f"remanage_node - node {node_name} remanaged.")
