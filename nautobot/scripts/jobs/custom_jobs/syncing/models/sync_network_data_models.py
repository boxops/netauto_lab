    ...
    def update(self, attrs):
        ...
        try:
            ip_address = IPAddress.objects.get(
                host=self.host, parent__namespace=self.adapter.job.namespace
            )
...
class SyncNetworkDataIPAddressToInterface(FilteredNautobotModel):
    ...
    def _get_queryset(cls, adapter: "Adapter"):
        """Get the queryset used to load the models data from Nautobot."""
        return IPAddressToInterface.objects.filter(
            interface__device__in=adapter.job.devices_to_load
        )
        ...
        try:
            vlan = VLAN(
                ...
                status=Status.objects.get(
                    name="Active"
                ),  # TODO: this can't be hardcoded, add a form input
            )
            vlan.validated_save()
        except ValidationError as err:
        ...
        if attrs.get("tagged_vlans"):
            try:
                interface = Interface.objects.get(
                    device__name=ids["device__name"], name=ids["name"]
                )
            except ObjectDoesNotExist:
                adapter.job.logger.error(
            ...
            if interface:
                try:
                    lag_interface = Interface.objects.get(
                        name=attrs["lag__interface__name"],
    ...
    def _get_and_assign_vrf(cls, adapter, attrs, interface):
        ...
        try:
            ...
            adapter.logger.error(
                f"Failed to assign device: {interface.device} to vrf: {vrf}, {err}"
            )
            raise diffsync_exceptions.ObjectNotCreated
        interface.vrf = vrf
