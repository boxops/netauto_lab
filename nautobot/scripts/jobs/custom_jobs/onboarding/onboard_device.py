...
class OnboardDevice:
    ...
    def _arista_eos(self):
        ...
        hostname_output = self.session.send_command("show hostname")
        hostname_pattern = r"Hostname:\s+(\S+)"
        self.hostname = self.find_match(hostname_pattern, hostname_output)
    ...
    def onboard(self):
        try:
            if self.platform.network_driver == "keymile_nos":
                ...
                self._tachyon_os()
            elif self.platform.network_driver == "arista_eos":
                self._arista_eos()
            else:
                raise Exception(
