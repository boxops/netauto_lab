...
def get_config_element(rule, config, obj, logger):
    ...
    if rule["obj"].config_type == ComplianceRuleConfigTypeChoice.TYPE_JSON:
        ...
        if rule["obj"].match_config:
            config_element = {
                k: config_json.get(k)
                for k in rule["obj"].match_config.splitlines()
                if k in config_json
            }
        else:
        ...
        if rule["obj"].match_config:
            try:
                config_element = get_xml_subtree_with_full_path(
                    config_xml, rule["obj"].match_config
                )
            except etree.XPathError as err:
                error_msg = (
                    f"`E3031:` Invalid XPath expression - `{rule['obj'].match_config}`"
                )
                logger.error(error_msg, extra={"object": obj})
                raise NornirNautobotException(error_msg) from err
        else:
            config_element = etree.tostring(
                config_xml, encoding="unicode", pretty_print=True
            )
        ...
        config_element = section_config(
            rule, config, obj.platform.network_driver_mappings["netutils_parser"]
        )

    else:
        error_msg = (
            f"`E3004:` There rule type ({rule['obj'].config_type}) is not recognized."
        )
        logger.error(error_msg, extra={"object": obj})
        raise NornirNautobotException(error_msg)
...
def run_compliance(  # pylint: disable=too-many-arguments,too-many-locals
    ...
    intended_path_template_obj = render_jinja_template(
        obj, logger, settings.intended_path_template
    )
...
def config_compliance(job):  # pylint: disable=unused-argument
    ...
    for settings in set(job.device_to_settings_map.values()):
        verify_settings(
            logger, settings, ["backup_path_template", "intended_path_template"]
        )
    try:
        with InitNornir(
