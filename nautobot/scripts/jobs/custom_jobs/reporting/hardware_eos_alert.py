...
from custom_jobs.operations.send_email import SendEmail
...
class HardwareEOLAlert(Job):
    ...
    def get_alert_list(self, alert_date):
        # Push the date filter into the DB — avoids loading all HardwareLCM rows
        # into Python memory just to discard most of them.
        return list(
            HardwareLCM.objects.filter(
                end_of_support__isnull=False,
                end_of_support__lte=alert_date,
            )
        )
