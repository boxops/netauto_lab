...
from custom_jobs.custom_configuration.custom_netutils.compliance import (
    feature_compliance,
)
...
MISSING_MSG = (
    ERROR_MSG
    + "Specifically the `{}` key was not found in value the get_custom_compliance function provided."
)
VALIDATION_MSG = (
    ERROR_MSG
    + "Specifically the key {} was expected to be of type(s) {} and the value of {} was not that type(s)."
)
...
def _get_custom_cli_compliance(obj):
    ...
    value = feature_compliance(
        ...
        obj.actual,
        obj.intended,
        obj.device.platform.network_driver_mappings.get("netutils_parser"),
    )
...
def _get_json_compliance(obj):
    ...
    diff = DeepDiff(
        obj.actual, obj.intended, ignore_order=obj.ordered, report_repetition=True
    )
    if not diff:
        compliance_int = 1
        compliance = True
    ...
    for val in ["compliance", "ordered"]:
        if compliance_details[val] not in [True, False]:
            raise ValidationError(
                VALIDATION_MSG.format(val, "Boolean", compliance_details[val])
            )
    if compliance_details["compliance_int"] not in [0, 1]:
        raise ValidationError(
            VALIDATION_MSG.format(
                "compliance_int", "0 or 1", compliance_details["compliance_int"]
            )
        )
    for val in ["missing", "extra"]:
        if not isinstance(compliance_details[val], str) and not _is_jsonable(
            compliance_details[val]
        ):
            raise ValidationError(
                VALIDATION_MSG.format(val, "String or Json", compliance_details[val])
            )
...
def _get_hierconfig_remediation(obj):
    ...
    if not hierconfig_os:
        raise ValidationError(
            f"platform {obj.network_driver} is not supported by hierconfig."
        )
    ...
    try:
        ...
        raise ValidationError(
            f"Platform {obj.network_driver} has no Remediation Settings defined."
        ) from err
...
class ComplianceRule(PrimaryModel):  # pylint: disable=too-many-ancestors
    ...
    feature = models.ForeignKey(
        to="ComplianceFeature", on_delete=models.CASCADE, related_name="feature"
    )
    ...
    def clean(self):
        ...
        if (
            self.config_type == ComplianceRuleConfigTypeChoice.TYPE_CLI
            and not self.match_config
        ):
            raise ValidationError(
                "CLI configuration set, but no configuration set to match."
            )
...
class ConfigCompliance(PrimaryModel):  # pylint: disable=too-many-ancestors
    ...
    device = models.ForeignKey(
        to="dcim.Device", on_delete=models.CASCADE, help_text="The device"
    )
    rule = models.ForeignKey(
        to="ComplianceRule", on_delete=models.CASCADE, related_name="rule"
    )
    compliance = models.BooleanField(blank=True)
    actual = models.JSONField(blank=True, help_text="Actual Configuration for feature")
    intended = models.JSONField(
        blank=True, help_text="Intended Configuration for feature"
    )
    # these three are config snippets exposed for the ConfigDeployment.
    remediation = models.JSONField(
        blank=True, help_text="Remediation Configuration for the device"
    )
    missing = models.JSONField(
        blank=True, help_text="Configuration that should be on the device."
    )
    extra = models.JSONField(
        blank=True, help_text="Configuration that should not be on the device."
    )
    ...
    def to_objectchange(
        ...
        *,
        related_object=None,
        object_data_extra=None,
        object_data_exclude=None,
        ...
        return ObjectChange(
            ...
            object_data=serialize_object(
                self, extra=object_data_extra, exclude=object_data_exclude
            ),
    ...
    def remediation_on_save(self):
        ...
        remediation_config = FUNC_MAPPER[
            self.rule.remediation_setting.remediation_type
        ](obj=self)
...
class GoldenConfig(PrimaryModel):  # pylint: disable=too-many-ancestors
    ...
    backup_config = models.TextField(
        blank=True, help_text="Full backup config for device."
    )
    ...
    intended_config = models.TextField(
        blank=True, help_text="Intended config for the device."
    )
    ...
    compliance_config = models.TextField(
        blank=True, help_text="Full config diff for device."
    )
    ...
    def get_dynamic_group_device_pks():
        ...
        for setting in GoldenConfigSetting.objects.all():
            ...
            gc_dynamic_group_device_queryset = (
                gc_dynamic_group_device_queryset | setting.dynamic_group.members
            )
...
class GoldenConfigSettingManager(BaseManager.from_queryset(RestrictedQuerySet)):
    ...
    def get_for_device(self, device):
        ...
        dynamic_group = device.dynamic_groups.exclude(
            golden_config_setting__isnull=True
        )
        if dynamic_group.exists():
            return (
                dynamic_group.order_by("-golden_config_setting__weight")
                .first()
...
class GoldenConfigSetting(PrimaryModel):  # pylint: disable=too-many-ancestors
    ...
    def clean(self):
        ...
        if ENABLE_SOTAGG and not self.sot_agg_query:
            raise ValidationError(
                "A GraphQL query must be defined when `ENABLE_SOTAGG` is True"
            )
    ...
    def get_jinja_template_path_for_device(self, device):
        ...
        if self.jinja_repository is not None:
            rendered_path = render_jinja2(
                template_code=self.jinja_path_template, context={"obj": device}
            )
            return (
                f"{self.jinja_repository.filesystem_path}{os.path.sep}{rendered_path}"
            )
        return None
...
class ConfigPlan(PrimaryModel):  # pylint: disable=too-many-ancestors
    ...
    plan_type = models.CharField(
        max_length=20, choices=ConfigPlanTypeChoice, verbose_name="Plan Type"
    )
    device = models.ForeignKey(
        to="dcim.Device",
        on_delete=models.CASCADE,
    ...
    config_set = models.TextField(
        help_text="Configuration set to be applied to device."
    )
