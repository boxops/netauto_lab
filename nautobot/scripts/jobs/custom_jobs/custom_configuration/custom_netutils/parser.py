...
# from banner import normalise_delimiter_caret_c
...
def paloalto_panos_clean_newlines(cfg: str) -> str:
    ...
    Examples:
        ...
        True
    """
    paloalto_panos_no_newline_cleanup_match = [
        "private-key",
        "public-key",
        "login-banner",
    ]
    ...
    newlines_cleaned_cfg = paloalto_panos_newline_regex.sub(
        lambda match: (
            match.group().replace("\n", " ")
            if not any(
                substring in match.group()
                for substring in paloalto_panos_no_newline_cleanup_match
            )
            else match.group()
        ),
        cfg,
...
class PaloAltoNetworksConfigParser(BaseSpaceConfigParser):
    ...
    def build_config_relationship(
        self,
    ) -> t.List[ConfigLine]:  # pylint: disable=too-many-branches
...
class KeymileNOSConfigParser(CiscoConfigParser, BaseSpaceConfigParser):
    """Keymile NOS config parser."""
    ...
    def banner_end(self) -> str:
        """Demarcate End of Banner char(s)."""
        raise NotImplementedError("Keymile NOS platform doesn't have a banner.")


class FiberstoreFSOSConfigParser(CiscoConfigParser, BaseSpaceConfigParser):
    """Fiberstore FSOS config parser."""
    ...
    def __init__(self, config: str):
        ...
        self.unique_config_lines: t.Set[ConfigLine] = set()
        self.same_line_children: t.Set[ConfigLine] = set()
        super(FiberstoreFSOSConfigParser, self).__init__(config)
    ...
    def _build_banner(self, config_line: str) -> t.Optional[str]:
        ...
        """
        config_line = normalise_delimiter_caret_c(self.banner_end, config_line)
        return super(FiberstoreFSOSConfigParser, self)._build_banner(config_line)
    ...
    def _update_config_lines(self, config_line: str) -> None:
        ...
        Returns:
            None
        """
        super(FiberstoreFSOSConfigParser, self)._update_config_lines(config_line)
        entry = self.config_lines[-1]
        if entry in self.unique_config_lines:
            self.same_line_children.add(entry)
        self.unique_config_lines.add(entry)
    ...
    def build_config_relationship(self) -> t.List[ConfigLine]:
        ...
        Examples:
            >>> from netutils.config.parser import FiberstoreFSOSConfigParser, ConfigLine
            >>> config = '''
            ... interface Ethernet1/1
            ...   vlan 10
            ...   no shutdown
            ... interface Ethernet1/2
            ...   shutdown'''
            >>> config_tree = FiberstoreFSOSConfigParser(str(config))
            >>> config_tree.build_config_relationship() == \
            ... [
            ...     ConfigLine(config_line='interface Ethernet1/1', parents=()),
            ...     ConfigLine(config_line='  vlan 10', parents=('interface Ethernet1/1',)),
            ...
            True
        """
        super(FiberstoreFSOSConfigParser, self).build_config_relationship()
        self._update_same_line_children_configs()
        return self.config_lines
