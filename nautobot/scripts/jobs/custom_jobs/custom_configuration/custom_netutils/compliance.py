...
from custom_jobs.custom_configuration.custom_netutils import parser
...
parser_map: t.Dict[str, t.Type[parser.BaseConfigParser]] = {
    ...
    "keymile_nos": parser.KeymileNOSConfigParser,
    "fiberstore_fsos": parser.FiberstoreFSOSConfigParser,
...
def _check_configs_differences(
    intended_cfg: str, actual_cfg: str, network_os: str
...
def _is_feature_ordered_compliant(
    feature_intended_cfg: str, feature_actual_cfg: str
) -> bool:
...
def compliance(
    ...
    for feature in features:
        ...
        compliance_results.update(
            {
                feature["name"]: feature_compliance(
...
def feature_compliance(
    ...
    feature_data["ordered_compliant"] = _is_feature_ordered_compliant(
        intended_cfg, backup_cfg
    )
    ...
    if feature_data["ordered_compliant"]:
        ...
        feature_data.update(
            _check_configs_differences(intended_cfg, backup_cfg, network_os)
        )
...
def find_unordered_cfg_lines(
    intended_cfg: str, actual_cfg: str
) -> t.Tuple[bool, t.List[t.Tuple[str, str]]]:
    ...
    if len(intended_lines) == len(actual_lines):
        ...
        unordered_lines = [
            (e1, e2) for e1, e2 in zip(intended_lines, actual_lines) if e1 != e2
        ]
